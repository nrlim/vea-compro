...
export async function getMitras(): Promise<{ data: Mitra[]; error: string | null }> {
  try {
    const data = await prisma.mitra.findMany({
      orderBy: { order: "asc" },
    });
    return { data: data as Mitra[], error: null };
  } catch (error: unknown) {
...
async function handleFileUpload(file: File | null, oldUrl?: string): Promise<string | null> {
  ...
  if (oldUrl && oldUrl.startsWith("/uploads/mitra/")) {
...
export async function createMitra(formData: FormData): Promise<{ error: string | null }> {
  try {
    ...
    // Get max order
    const maxOrderMitra = await prisma.mitra.findFirst({
      orderBy: { order: 'desc' },
      select: { order: true }
    });
    const order = (maxOrderMitra?.order ?? -1) + 1;
    ...
    revalidatePath("/internal-admin/mitra");
    revalidatePath("/");
    return { error: null };
  } catch (error: unknown) {
...
export async function updateMitra(id: string, formData: FormData): Promise<{ error: string | null }> {
  try {
    const session = await getSession();
    if (!session) return { error: "Unauthorized request" };
...
export async function deleteMitra(id: string): Promise<{ error: string | null }> {
  try {
    const session = await getSession();
    if (!session) return { error: "Unauthorized request" };

    const mitra = await prisma.mitra.findUnique({ where: { id } });
    if (mitra && mitra.logoUrl && mitra.logoUrl.startsWith("/uploads/mitra/")) {
      const fileName = path.basename(mitra.logoUrl);
      const filePath = path.join(process.cwd(), "public", "uploads", "mitra", fileName);
...
export async function reorderMitras(items: { id: string; order: number }[]): Promise<{ error: string | null }> {
  try {
    const session = await getSession();
    if (!session) return { error: "Unauthorized request" };
